...
(function() {
  ...
  function updateBarContent(res) {
    ...
    const isGeminiSite = window.location.hostname.includes("gemini");
    const providerName = isGeminiSite ? "Gemini" : "Claude";
    ...
    recommendationBar.innerHTML = `
      <div class="squeeze-router-info">
        ...
        <div class="squeeze-router-text">
          <span class="squeeze-router-title">Run this prompt on <strong>${providerName} ${recommendedLabel}</strong>?</span>
  ...
  function showToast(modelName) {
    ...
    toast.innerHTML = `
      ...
      <span class="squeeze-router-toast-text">Routed to <span class="model-name">${providerName} ${modelName.toUpperCase()}</span></span>
    `;
