...
(function() {
  ...
  function findAndInjectWidget() {
    ...
    // Claude's input is a div with contenteditable="true" or role="textbox" or .ProseMirror
  ...
  function injectButton(inputElement) {
    ...
    // Try to find Claude's model chooser button inside the bottom input container
    ...
    if (modelBtn && modelBtn.parentNode) {
      ...
      if (attachButton && attachButton.parentNode) {
        attachButton.parentNode.appendChild(buttonsGroup);
  ...
  // Find the model selection dropdown button STRICTLY inside Claude's bottom input box
  function findModelChooser() {
    ...
    while (container && depth < 5) {
      ...
      const textButtons = containerButtons.filter(btn => {
        const text = (btn.innerText || "").trim();
        return text.length > 0 && !text.includes("chat with");
      });
  ...
  function generateHeuristicSummary() {
    ...
    // ── Try 100% Local TextRank TF-IDF Graph Summarizer Engine ──
    if (typeof TextRankSummarizer !== "undefined" && TextRankSummarizer.summarizeTurns) {
      try {
        const textRankResult = TextRankSummarizer.summarizeTurns(turns);
        if (textRankResult && textRankResult.summary) {
          return textRankResult.summary;
        }
      } catch (e) {
        console.warn("[Squeeze] TextRank summarization fallback:", e);
      }
    }
  ...
  // Listen for Context Capture / Summarizer requests from Popup
  if (isExtensionValid() && typeof chrome !== "undefined" && chrome.runtime && chrome.runtime.onMessage) {
    chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
      if (request && (request.action === "captureContext" || request.action === "getChatSummary")) {
        try {
          const summaryText = generateHeuristicSummary();
