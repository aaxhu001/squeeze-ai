  ...
  if (message.action === "optimizePrompt") {
    ...
    chrome.storage.local.get([
      ...
      try {
        ...
        const { optimized, rulesApplied } = optimizeLocally(processedPrompt, optMode, rules);
        if (pdfDistilled && !rulesApplied.includes("PDF Concept Distillation & Restatement Pruning")) {
          rulesApplied.unshift("PDF Concept Distillation & Restatement Pruning");
        }
