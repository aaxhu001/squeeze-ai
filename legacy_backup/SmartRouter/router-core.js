...
(function() {
  ...
  function classifyPrompt(promptText, context = {}) {
    ...
    const result = {
      ...
      reason: buildReason(simpleHits, moderateHits, complexHits, wordCount),
  ...
  function buildReason(simpleHits, moderateHits, complexHits, wordCount) {
    const reasons = [];

    if (complexHits >= 3) reasons.push("multiple complex keywords");
    else if (complexHits >= 1) reasons.push("advanced engineering request");
    
    if (simpleHits >= 2) reasons.push("simple task phrasing");
    
    if (wordCount <= 10) reasons.push(`only ${wordCount} words`);
