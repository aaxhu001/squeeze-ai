...
(function() {
  ...
  // Check if current page is an unauthenticated login / sign-in screen
  function isAuthOrLoginPage() {
    const path = window.location.pathname.toLowerCase();

    // URL path checks for auth/login pages across host sites
    if (path.includes("/login") || path.includes("/auth") || path.includes("/signup") || path.includes("/sign-in") || path.includes("/about")) {
      return true;
    }

    // Check for explicit login forms on dedicated auth cards (only if no chat prompt editor exists)
    const loginForm = document.querySelector('form[action*="login" i], form[action*="auth" i]');
    if (loginForm && !document.querySelector('div[contenteditable="true"]')) {
      return true;
    }
  ...
  function removeWidgetButtons() {
    if (triggerButton) { triggerButton.remove(); triggerButton = null; }
    if (pdfButton) { pdfButton.remove(); pdfButton = null; }
    if (undoButton) { undoButton.remove(); undoButton = null; }
    const vaultBadge = document.getElementById("squeezeVaultBadge");
    if (vaultBadge) vaultBadge.remove();
    const group = document.querySelector(".squeeze-buttons-group");
    if (group) group.remove();
    if (circleWidgetWrapper) { circleWidgetWrapper.remove(); circleWidgetWrapper = null; }
  }
  ...
  function findInputElement() {
    ...
    // Claude & default fallback (STRICT to chat prompt editor, never login inputs)
    const claudePromptInput = document.querySelector('div[contenteditable="true"].ProseMirror') ||
    ...
    if (claudePromptInput) return claudePromptInput;

    // Fallback: only match contenteditable if inside a main prompt form/fieldset (not login/auth forms)
    const fallback = document.querySelector('form div[contenteditable="true"]') ||
                     document.querySelector('fieldset div[contenteditable="true"]');

    if (fallback && !fallback.closest('form[action*="login"]') && !fallback.closest('form[action*="auth"]')) {
      return fallback;
    }
  ...
  function findAndInjectWidget() {
    if (!isExtensionValid() || isAuthOrLoginPage()) {
      removeWidgetButtons();
      return;
    }
      ...
      <div class="squeeze-modal-card">
        <div class="tm-modal-header">
          <div class="tm-modal-logo">
            <svg class="squeeze-logo-svg animating" width="22" height="22" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg" style="vertical-align: middle;">
              <rect class="squeeze-logo-box" width="40" height="40" rx="10" fill="#07090E" stroke="rgba(255, 255, 255, 0.15)" stroke-width="1.5"/>
              <path class="squeeze-logo-star" d="M 20 7 Q 20 20 33 20 Q 20 20 20 33 Q 20 20 7 20 Q 20 20 20 7 Z" fill="#F5C518"/>
            </svg>
  ...
  function renderComparison(data) {
    ...
    const logoImg = modalContainer.querySelector(".tm-modal-logo .animating, .tm-modal-logo svg, .tm-modal-logo img");
    if (logoImg) {
  ...
  let circleWidgetWrapper = null;
  let circlePopoverOpen = false;

  function findAttachmentPlusButton(inputElement) {
    const container = inputElement ? (inputElement.closest('form') || inputElement.closest('fieldset') || inputElement.closest('div[class*="input" i]') || inputElement.closest('div[class*="prompt" i]') || inputElement.closest('div[class*="composer" i]') || inputElement.parentElement?.parentElement) : document;
    ...
    // Fallback search inside container for button with SVG or + text
    const btns = container.querySelectorAll('button, div[role="button"]');
    for (const b of btns) {
      const aria = (b.getAttribute('aria-label') || '').toLowerCase();
      const title = (b.getAttribute('title') || '').toLowerCase();
      const text = (b.innerText || '').toLowerCase().trim();
      if (aria.includes('attach') || aria.includes('add') || aria.includes('file') || aria.includes('content') || title.includes('attach') || text === '+' || aria.includes('plus')) {
        return b;
      }
    }
  ...
  function injectCircleContextWidget() {
    if (isAuthOrLoginPage()) {
      if (circleWidgetWrapper) {
        circleWidgetWrapper.remove();
        circleWidgetWrapper = null;
      }
      return;
    }
    ...
    safeStorageGet(["extensionEnabled"], (data) => {
      ...
      const plusBtn = findAttachmentPlusButton(inputElement);

      if (!circleWidgetWrapper || !document.contains(circleWidgetWrapper)) {
        circleWidgetWrapper = document.createElement("div");
        circleWidgetWrapper.className = "squeeze-circle-widget-wrapper";

        if (plusBtn && plusBtn.parentNode) {
          // Insert right beside the plus (+) attachment button inside the chat input box
          plusBtn.parentNode.insertBefore(circleWidgetWrapper, plusBtn.nextSibling);
        } else {
          // Fallback: place before model chooser or squeeze buttons group inside the prompt card
          const modelBtn = findModelChooser();
          if (modelBtn && modelBtn.parentNode) {
            modelBtn.parentNode.insertBefore(circleWidgetWrapper, modelBtn);
          } else {
            const wrapper = inputElement.closest('form') || inputElement.closest('fieldset') || inputElement.parentElement;
            if (wrapper) wrapper.appendChild(circleWidgetWrapper);
          }
        }
      } else if (plusBtn && plusBtn.parentNode && circleWidgetWrapper.parentNode !== plusBtn.parentNode) {
        plusBtn.parentNode.insertBefore(circleWidgetWrapper, plusBtn.nextSibling);
      }

      updateCircleContextWidget();
    });
  }
  ...
  let circlePopoverElement = null;

  function createOrGetPopover() {
    if (!circlePopoverElement || !document.body.contains(circlePopoverElement)) {
      circlePopoverElement = document.createElement("div");
      circlePopoverElement.className = "squeeze-context-popover";
      circlePopoverElement.id = "squeezeContextPopover";
      document.body.appendChild(circlePopoverElement);

      circlePopoverElement.addEventListener("click", (e) => {
        e.stopPropagation();
      });
    }
    return circlePopoverElement;
  }

  function togglePopover(circleBtn) {
    circlePopoverOpen = !circlePopoverOpen;
    const popover = createOrGetPopover();
  ...
  async function updateCircleContextWidget() {
    if (!circleWidgetWrapper) return;
    ...
    const formattedUsed = formatTokenCount(conversationTokens);
    const formattedLimit = formatTokenCount(localLimit);

    const radius = 12;
    const circumference = 2 * Math.PI * radius; // 75.398
    const strokeOffset = circumference - (localPercentage / 100) * circumference;

    let strokeColor = "#D97757"; // Claude signature logo orange
    if (localPercentage >= 85) {
      strokeColor = "#EF4444";
    } else if (localPercentage >= 65) {
      strokeColor = "#F59E0B";
    }

    // 1. Ensure circle button exists inside wrapper without blowing away DOM on every tick
    let circleBtn = circleWidgetWrapper.querySelector("#squeezeCircleBtn");
    if (!circleBtn) {
      circleWidgetWrapper.innerHTML = `
        <div class="squeeze-circle-btn" id="squeezeCircleBtn" title="Click to view context & quota usage">
          <svg class="squeeze-circle-svg" width="30" height="30" viewBox="0 0 30 30">
            <circle cx="15" cy="15" r="${radius}" fill="none" stroke="rgba(255,255,255,0.12)" stroke-width="2.5" />
            <circle id="squeezeCircleStroke" cx="15" cy="15" r="${radius}" fill="none" stroke="${strokeColor}" stroke-width="2.5"
          ...
          <span class="squeeze-circle-text" id="squeezeCircleText">${localPercentage}%</span>
        </div>
      `;
      circleBtn = circleWidgetWrapper.querySelector("#squeezeCircleBtn");
      circleBtn.addEventListener("click", (e) => {
        e.stopPropagation();
        togglePopover(circleBtn);
      });
    } else {
      const strokeEl = circleBtn.querySelector("#squeezeCircleStroke");
      const textEl = circleBtn.querySelector("#squeezeCircleText");
      if (strokeEl) {
        strokeEl.setAttribute("stroke", strokeColor);
        strokeEl.setAttribute("stroke-dashoffset", strokeOffset);
      }
      if (textEl) {
        textEl.textContent = `${localPercentage}%`;
      }
    }

    // 2. If popover is open, update popover contents
    if (circlePopoverOpen) {
      const popover = createOrGetPopover();
      ...
      if (platform === "claude") {
        ...
        if (quota && quota.five_hour) {
          ...
          quotaInfo = `
            <div class="squeeze-popover-quota-row">
              <div class="quota-row-label">
                <span class="tm-usage-dot dot-${getPercentStatusClass(utilization)}"></span>
                <span>5-Hour Quota:</span>
              </div>
              <div class="quota-row-val">${formatTokenCount(usedTokens)} / ${formatTokenCount(maxTokens)} (${utilization}%)</div>
            </div>
            <div class="quota-row-reset">${resetString}</div>
          `;
        }
      }
      ...
      if (conversationTokens > 30000) {
        summaryNudge = `
          <div class="squeeze-popover-nudge">
            <div class="nudge-text">High Context depth (${formattedUsed} tokens). Start a new chat to cut latency!</div>
            <button class="tm-nudge-btn tm-btn-gold" id="tmHeuristicSummaryBtnPopover">Copy Summary & New Chat</button>
          </div>
        `;
      }

      popover.innerHTML = `
        <div class="squeeze-popover-card">
          <div class="squeeze-popover-main-stat">
            <div class="popover-percent-title">${localPercentage}% context used</div>
            <div class="popover-tokens-detail">${formattedUsed} / ${formattedLimit} tokens</div>
          </div>

          <div class="squeeze-popover-bar-track">
            <div class="squeeze-popover-bar-fill" style="width: ${localPercentage}%; background-color: ${strokeColor};"></div>
          </div>
      ...
      const localBtnPopover = popover.querySelector("#tmHeuristicSummaryBtnPopover");
      if (localBtnPopover) {
        localBtnPopover.addEventListener("click", (e) => {
          e.stopPropagation();
  ...
  // Global click handler to close popover on outside click
  document.addEventListener("click", (e) => {
    if (circlePopoverOpen) {
      const popover = document.getElementById("squeezeContextPopover");
      const circleBtn = circleWidgetWrapper ? circleWidgetWrapper.querySelector("#squeezeCircleBtn") : null;
      if (popover && !popover.contains(e.target) && circleBtn && !circleBtn.contains(e.target)) {
