...
function optimizeLocally(text, mode, rules) {
  ...
  // 1. Isolate code blocks and other sensitive markdown syntax
  const codeBlocks = [];
  let placeholderCount = 0;
  const protectionRegex = /(```[\s\S]*?```|`[^`\n]+`|<[^>]+>|!\[.*?\]\(.*?\)|\[.*?\]\(.*?\)|https?:\/\/[^\s<>\)\]"']+|www\.[^\s<>\)\]"']+)/g;

  let protectedText = text.replace(protectionRegex, (match) => {
    const placeholder = `__SQUEEZE_PLACEHOLDER_${placeholderCount}__`;
  ...
  if (rules.ruleStripGreetings) {
    const greetings = [
      // 1. Common confirmation pads (strip entire sentence/clause of helper chats)
      ...
      { regex: /\blet\s+me\s+know\s+if\s+you\s+have\s+(?:any\s+)?questions[.,!?]*\s*/gi, label: "Politeness padding removal" },
      { regex: /\blet\s+me\s+know\s+what\s+you\s+think[.,!?]*\s*/gi, label: "Politeness padding removal" },
      ...
      // 4. Polite auxiliary queries & starts
      ...
      // 5. Polite please (preserves punctuation and spaces to prevent join bugs)
